...
export function AIChat() {
  ...
  useEffect(() => {
    if (user?.email) {
      ...
      // Botpress webchat URL with user configuration
      const baseUrl = config.botpress.webchatUrl;
      const configUrl = config.botpress.configUrl;
      
      // Build the chat URL with user context
      const url = `${baseUrl}?configUrl=${encodeURIComponent(configUrl)}&userId=${userIdentifier}`;
